...
function getBalanceIndex(arr) {
  ...
  for (let i = 0; i < arr.length - 1; i += 1) {
    if (sumLeft === sum - sumLeft - arr[i]) return i;
    sumLeft += arr[i];
  }
  return -1;
}
...
function getSpiralMatrix(size) {
  ...
  for (let i = 0; i < size * size; i += 1) {
    if (way === 'right') {
      column += 1;
      if (column === size - 1 || arr[row][column + 1]) {
        way = 'down';
      }
    } else if (way === 'down') {
      row += 1;
      if (row === size - 1 || arr[row + 1][column]) {
        way = 'left';
      }
    } else if (way === 'left') {
      column -= 1;
...
function sortByAsc(arr) {
  ...
  while (done < arr.length) {
    for (let i = 0; i < arr.length; i += done * 2) {
      left = i;
      right = i + done;
      for (let j = 0; j < done * 2; j += 1) {
        if (right < arr.length && right < i + 2 * done) {
          if (left < i + done) {
        ...
        } else if (left < i + done && i < arr.length) {
          temp[i + j] = arr[left];
          left += 1;
        } else {
          break;
...
function shuffleChar(str, iterations) {
  ...
  let pow = 1;
  const mod = str.length + (str.length % 2) - 1;
  let temp = 2;
  let iter = iterations;
  while (iter > 0) {
    if (iter % 2) {
  ...
  if (mod < str.length) ans += str[str.length - 1];
  return ans;
}
